...
u'''Multiprocessing can be done with refprop.py module provided that the setup
...
class _MultiRefProp(mp.Process):
    u'''initiate multiprocessing for refprop,
...
class MultiRPError(RefpropError):
    u'General error for multiRP'
    pass

class SetupError(MultiRPError):
    u'Raise input error when Setups are blocked'
...
class FluidModel(refprop.FluidModel):
    u'''return array of fluid model'''
    pass

class SetWarning():
    u'Return RefpropdllWarning status (on / off)'
    def __repr__(self):
        return unicode(refprop.SetWarning())
    @staticmethod
...
class SetError():
    ...
    def __repr__(self):
        return unicode(refprop.SetError())
    @staticmethod
    def on(prop=None, mRP=None):
...
class SetErrorDebug():
    ...
    def __repr__(self):
        return unicode(refprop.SetErrorDebug())
    @staticmethod
    def on(prop=None, mRP=None):
...
#functions
def multirefprop():
    u'initiate multiprocessing variables'
    global mRP
    ...
    if mp.current_process()._parent_pid == None:
        ...
        if not u'mRP' in globals():
            _mRP = _MultiRefProp()
            mRP = {u'sem':_mRP.sem,
                   u'process':_mRP.process,
                   u'result':_mRP.result, u'ppipe':_mRP.ppipe,
                   u'cpipe':_mRP.cpipe}
    else:
        raise MultiRPChildError(
        u'Only parent process can initiate mRP variables')

    return mRP

def run_mRP(processlist):
    u'start, close and check child processes'
    ...
    for each in processlist:
        if each.exitcode != 0:
            raise MultiRPChildError(u'child error in ' + each.name +
                                    u' exitcode = ' + unicode(each.exitcode))


def _multirefprop(_mRP):
    u'Set parent multiprocessing variables as globals'
    global sem, process, result, mRP, ppipe, cpipe
    sem = _mRP[u'sem']
    result = _mRP[u'result']
    process = _mRP[u'process']
    mRP = _mRP
    cpipe = _mRP[u'cpipe']
    ppipe = _mRP[u'ppipe']

def ppip():
    u'return parent pipe'
    if u'mRP' in globals():
        return mRP[u'pipe']
    else:
        raise MultiRPInputError(u'parent pipe can not be return without a' +
                                 u' multirefprop() call')
...
def _checksetupblock(name): ##################################################################perhaps this whole function can be ommitted, but carefully as settings are passed through
    if mp.current_process()._parent_pid != None:
        ...
        raise SetupError(u'function "' + unicode(name) +
                          u'" is blocked for multiRP child processes')
    elif len(mp.active_children()) > 1:
        #raise error if parent process request for setup while child process(es)
        #are active.
        raise SetupError(
            u'function "' + unicode(name) +
            u'" is blocked while multiRP child processes are active')


#functions from refprop.py
def setpath(path=u'c:/program files/refprop/'):
    u'''Set Directory to refprop root containing fluids and mixtures.'''
    refprop.setpath(path)

def fluidlib():
    u'''Displays all fluids and mixtures available on root directory.'''
    refprop.fluidlib()
...
def setup_setting(mRP=None):
    u"""Returns current setup settings"""
    ...
    len(mp.active_children()) <= 1):
        ...
        if u'mRP' not in globals():
            raise MultiRPInputError(
            u'"mRP" input required when multiprocessing')
...
def _test():
    u"""execute detailed test run of multiRP"""
    import rptest
    rptest.settest(u'multiRP')

def test(criteria = 0.00001):
    u'''verify that the user's computer is returning proper calculations'''
    global testresult
...
def limitx(x, htype=u'EOS', t=0, D=0, p=0, prop=None, mRP=None):
...
def limitk(htype=u'EOS', icomp=1, t=u'tnbp', D=0, p=0, prop=None, mRP=None):
...
def limits(x, htype = u'EOS', prop=None, mRP=None):
    u'''Returns limits of a property model as a function of composition.'''
    def _rpfunc():
...
def _mRPpipe(fluid, mRP):
    ...
    for each in xrange(100):
        #put value into pipe
        mRP[u'cpipe'].send(
            press(303 + each, 58, [0.4, 0.6])[u'p'])
    mRP[u'cpipe'].close()
...
if __name__ == u'__main__':
    ...
    H2O = setup(u'def', u'WATER')
    H2O_NH3 = setup(u'def', u'AMMONIA', u'WATER')
    CH4 = setup(u'def', u'METHANE')
    CH4_C2H6 = setup(u'def', u'METHANE', u'ETHANE')

    #various refprop functions named
    peen = mRP[u'process'](target=critp, args=([1], H2O, mRP))
    ptwee = mRP[u'process'](target=critp, args=([0.4, 0.6],),
                           kwargs={u'prop':H2O_NH3, u'mRP':mRP})#alternative input
    pdrie = mRP[u'process'](target=critp, args=([1], CH4, mRP))
    pvier = mRP[u'process'](target=critp, args=([0.35, 0.65], CH4_C2H6, mRP))
    qeen = mRP[u'process'](target=critp, args=([1], H2O, mRP))
    qtwee = mRP[u'process'](target=critp, args=([0.4, 0.6], H2O_NH3, mRP))
    qdrie = mRP[u'process'](target=critp, args=([1], CH4, mRP))
    qvier = mRP[u'process'](target=critp, args=([0.35, 0.65], CH4_C2H6, mRP))
    ween = mRP[u'process'](target=critp, args=([1], H2O, mRP))
    wtwee = mRP[u'process'](target=critp, args=([0.4, 0.6], H2O_NH3, mRP))
    wdrie = mRP[u'process'](target=critp, args=([1], CH4, mRP))
    wvier = mRP[u'process'](target=critp, args=([0.35, 0.65], CH4_C2H6, mRP))
    reen = mRP[u'process'](target=critp, args=([1], H2O, mRP))
    rtwee = mRP[u'process'](target=critp, args=([0.4, 0.6], H2O_NH3, mRP))
    rdrie = mRP[u'process'](target=critp, args=([1], CH4, mRP))
    rvier = mRP[u'process'](target=critp, args=([0.35, 0.65],
                                               CH4_C2H6, mRP))
    sfun = mRP[u'process'](target=_mRPpipe, args=(H2O_NH3, mRP))#pipe
    ...
    for each in processlist:
        if each.name in mRP[u'result']:
            print mRP[u'result'][each.name] #only returns the last refprop function results unless tweaked at function call

    #loop untill pipe is empty
    while mRP[u'ppipe'].poll():#parentpipe
        #print value from pipe
        print mRP[u'ppipe'].recv()
